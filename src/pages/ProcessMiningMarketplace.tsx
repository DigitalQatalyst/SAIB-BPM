...
const useCases = ['All Use Cases', 'Loan Processing', 'Compliance', 'Customer Onboarding', 'Document Approval', 'Risk Management', 'Process Optimization', 'Performance Monitoring', 'Loan Disbursement', 'Regulatory Change', 'Customer Feedback', 'Version Control', 'Risk Assessment'];
const complexities = ['All Complexities', 'Low', 'Medium', 'High'];
const departments = ['All Departments', 'P&P'];
const integrations = ['All Integrations', 'Dynamics 365', 'SharePoint', 'Office 365', 'Visio', 'Snowflake', 'CRM', 'OneDrive', 'Teams'];
const features = ['All Features', 'Real-Time Analysis', 'Lifecycle Tracking', 'Process Design', 'Strategy Alignment', 'Heatmap', 'Training', 'Collaboration'];
const aiPoweredOptions = ['All', 'Yes', 'No'];
const riskLevels = ['All Risk Levels', 'Low', 'Medium', 'High'];
const sortOptions = ['Relevance', 'Newest', 'A-Z'];
const ProcessMiningMarketplace = () => {
  const {
    t
  } = useLanguage();
  ...
  const filteredTools = processMiningTools.filter(tool => {
    ...
    return matchesSearch && matchesIndustry && matchesUseCase && matchesComplexity && matchesDepartment && matchesIntegration && matchesFeature && matchesAIPowered && matchesRiskLevel;
  });
  ...
  const getTagsForTool = (tool: any) => {
    ...
    if (tags.length < 3) {
      ...
      if (tool.description.toLowerCase().includes('workflow')) {
        tags.push({
          text: 'Workflow',
          className: 'text-gray-700 bg-gray-100'
        });
  ...
  return <div className="bg-white pb-16">
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${comparisonTools.length > 0 ? 'pt-28' : ''}`}>
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Process Mining</h1>
          <p className="mt-2 text-gray-600">
            Explore tools and services for analyzing and optimizing business
            processes to improve efficiency and compliance.
          </p>
        </div>
        {/* Search bar */}
        <div className="mb-8">
          <div className="relative">
            <input type="text" className="w-full pl-10 pr-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#FECC0E] focus:border-[#FECC0E]" placeholder="Search process mining tools and templates..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          ...
          <div className="md:w-3/4">
            ...
            {paginatedTools.length === 0 ? <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
              ...
              return <div key={tool.id} className="relative bg-white rounded-lg border border-gray-200 overflow-hidden h-full transition-all duration-300 hover:border-[#FECC0E] hover:shadow-md group cursor-pointer">
                      ...
                      <Link to={`/process-mining/${tool.id}`} className="p-6 flex flex-col h-full">
...
export default ProcessMiningMarketplace;