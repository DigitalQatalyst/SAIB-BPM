...
import { ArrowLeft, Clock, FileBarChart, CheckCircle, AlertTriangle, Download, BookmarkCheck, Bookmark, ChevronRight, ChevronDown, ChevronUp, Share2, PieChart, BarChart, Users, Calendar, Bell, Star, Scale, Check, Loader, Shield, AlertOctagon } from 'lucide-react';
import { useLanguage } from '../context/LanguageContext';
import { riskManagementTools } from '../data/riskManagementData';
import { useComparison } from '../context/ComparisonContext';
import ComparisonPanel from '../components/comparison/ComparisonPanel';
import ComparisonModal from '../components/comparison/ComparisonModal';
const RiskManagementDetails = () => {
  const {
    id
  } = useParams<{
    id: string;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const foundTool = riskManagementTools.find(t => t.id === numericId);
      ...
      const related = riskManagementTools.filter(t => {
  ...
  if (error) {
    return <div className="bg-gray-50 min-h-screen">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <Link to="/risk-management" className="inline-flex items-center text-sm font-medium text-[#9E800D] hover:text-[#FECC0E] mb-6 transition-colors">
          ...
          <div className="text-center py-16 bg-white rounded-lg border border-gray-200">
            ...
            <button onClick={() => navigate('/risk-management')} className="bg-[#FECC0E] hover:bg-[#FECC0E]/90 text-gray-900 font-medium py-2 px-6 rounded-md transition-colors shadow-sm">
              Browse All Tools
            </button>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center py-16 bg-gray-50 rounded-lg border border-gray-200">
            ...
            <p className="text-gray-600 mb-6">
              The risk management tool you're looking for doesn't exist or has
              been removed.
            </p>
            <button onClick={() => navigate('/risk-management')} className="bg-[#FECC0E] hover:bg-[#FECC0E]/90 text-gray-900 font-medium py-2 px-6 rounded-md transition-colors shadow-sm">
              Browse All Tools
  ...
  const generateBenefits = () => {
    ...
    if (tool.aiPowered) {
      benefits.push({
        ...
        description: 'Leverages artificial intelligence to provide intelligent risk analysis and recommendations'
      });
    }
    if (tool.realTime) {
      benefits.push({
        title: 'Real-time monitoring',
        description: 'Provides immediate visibility into risk metrics and emerging threats'
      });
    }
    if (tool.useCase === 'Risk Assessment') {
      benefits.push({
        title: 'Comprehensive risk assessment',
        description: 'Enables thorough identification and evaluation of risks across processes'
      });
    }
    if (tool.useCase === 'Regulatory Change') {
      benefits.push({
        title: 'Regulatory change management',
        description: 'Streamlines adaptation to new regulatory requirements and ensures compliance'
      });
    }
    if (tool.useCase === 'Operational Risk') {
      benefits.push({
        title: 'Operational risk reduction',
        description: 'Minimizes operational risks through systematic identification and mitigation'
      });
    }
    if (tool.feature === 'Risk Mitigation') {
      benefits.push({
        title: 'Effective risk mitigation',
        description: 'Provides structured approaches to reduce the impact and likelihood of identified risks'
      });
    }
    if (tool.feature === 'Heatmap') {
      benefits.push({
        title: 'Visual risk assessment',
        description: 'Intuitive visualization of risk levels across processes and departments'
      });
    }
    if (tool.feature === 'Strategy Alignment') {
      benefits.push({
        title: 'Strategic risk alignment',
        description: 'Aligns risk management activities with organizational goals and strategies'
      });
    }
    if (tool.title.toLowerCase().includes('dashboard')) {
      benefits.push({
        title: 'Comprehensive risk dashboards',
        description: 'Customizable dashboards that provide a holistic view of risk metrics'
      });
    }
    if (tool.department) {
      benefits.push({
        title: `${tool.department} risk optimization`,
        description: `Specifically designed for ${tool.department} teams to enhance risk management`
      });
    }
    // Add generic benefits if we don't have enough
    if (benefits.length < 3) {
      benefits.push({
        title: 'Risk management efficiency',
        description: 'Improves risk management processes and reduces administrative overhead'
      });
    }
    if (benefits.length < 3) {
      benefits.push({
        title: 'Data-driven risk decisions',
        description: 'Enables evidence-based risk decisions through comprehensive metrics'
      });
    }
    if (benefits.length < 3) {
      benefits.push({
        title: 'Enhanced risk visibility',
        description: 'Increases visibility of risks across the organization for better decision making'
      });
    }
  ...
  const getUseCases = () => {
    ...
    if (tool.industry) {
      useCases.push({
        title: `${tool.industry} Risk Management`,
        description: `Tailored for risk management in ${tool.industry.toLowerCase()} environments`
      });
    }
    if (tool.department) {
      useCases.push({
        title: `${tool.department} Risk Management`,
        description: `Drive risk management excellence within ${tool.department} teams and functions`
      });
    }
    if (tool.riskLevel === 'High') {
      useCases.push({
        title: 'Critical Risk Areas',
        description: 'Track and manage high-impact risk areas requiring close oversight'
      });
    }
    if (tool.feature === 'Heatmap') {
      useCases.push({
        title: 'Risk Hotspot Identification',
        description: 'Visually identify and address risk hotspots and mitigation opportunities'
      });
    }
    if (tool.title.toLowerCase().includes('dashboard')) {
      useCases.push({
        title: 'Risk Reporting',
        description: 'Generate comprehensive risk reports for leadership and stakeholders'
      });
    }
    ...
    if (useCases.length < 3) {
      useCases.push({
        ...
        description: 'Identify potential risks before they impact business operations'
      });
    }
    if (useCases.length < 3) {
      useCases.push({
        title: 'Risk Mitigation Planning',
        description: 'Develop structured plans to address and mitigate identified risks'
      });
    }
    if (useCases.length < 3) {
      useCases.push({
        title: 'Risk Monitoring',
        description: 'Continuously monitor risk indicators and trends'
      });
    }
  ...
  const getImplementationSteps = () => {
    ...
    if (tool.title.toLowerCase().includes('template')) {
      steps.push({
        ...
        description: 'Select and customize the template for your specific risk management needs'
      });
      steps.push({
        title: 'Risk Framework Mapping',
        description: 'Map your existing risk frameworks to the template structure'
      });
      steps.push({
        title: 'Configuration',
        description: 'Configure risk thresholds and reporting parameters'
      });
      steps.push({
        title: 'Testing',
        description: 'Test with historical risk data to validate accuracy'
      });
      steps.push({
        title: 'Deployment',
      ...
      steps.push({
        title: 'Risk Indicator Definition',
        description: 'Identify key risk indicators (KRIs) to monitor'
      });
      steps.push({
        title: 'Data Connection',
        description: 'Connect risk data sources to the dashboard'
      });
      steps.push({
        title: 'Visualization Setup',
        description: 'Configure risk visualization and reporting preferences'
      });
      steps.push({
        title: 'Alert Configuration',
        description: 'Set up alerts and notifications for key risk thresholds'
      });
      steps.push({
        title: 'Stakeholder Access',
        description: 'Share dashboard with risk management stakeholders'
      });
    } else if (tool.aiPowered) {
      steps.push({
        title: 'Data Preparation',
        description: 'Prepare historical risk data for AI training'
      });
      steps.push({
        title: 'AI Configuration',
        description: 'Configure AI parameters and risk detection thresholds'
      });
      steps.push({
        title: 'Model Training',
        description: 'Train the model with your risk data'
      });
      steps.push({
        title: 'Validation',
        description: 'Validate risk predictions against expert knowledge'
      });
      steps.push({
      ...
      steps.push({
        ...
        description: 'Define risk management objectives and scope'
      });
      steps.push({
        title: 'Data Collection',
        description: 'Gather and prepare risk data from source systems'
      });
      steps.push({
        title: 'Tool Configuration',
        description: 'Configure the tool according to your specific risk requirements'
      });
      steps.push({
        title: 'Risk Analysis',
        description: 'Analyze results and identify mitigation opportunities'
      });
      steps.push({
        title: 'Implementation',
        description: 'Implement risk management changes and monitor outcomes'
      });
    }
    return steps;
  };
  ...
  const getFeatures = () => {
    ...
    if (tool.aiPowered) {
      features.push({
        ...
        description: 'Intelligent risk insights and recommendations powered by machine learning'
      });
    }
    if (tool.realTime) {
      features.push({
        ...
        title: 'Real-time monitoring',
        description: 'Live tracking and analysis of risk metrics'
      });
    }
    if (tool.integration) {
      features.push({
    ...
    if (tool.title.toLowerCase().includes('dashboard')) {
      features.push({
        ...
        description: 'Customizable visual displays of key risk metrics and KRIs'
      });
    }
    if (tool.title.toLowerCase().includes('heatmap')) {
      features.push({
        icon: <AlertOctagon className="h-5 w-5 text-[#FECC0E]" />,
        title: 'Risk heatmap visualization',
        description: 'Visual representation of risk levels across different areas'
      });
    }
    if (tool.title.toLowerCase().includes('analyzer')) {
      features.push({
        icon: <Scale className="h-5 w-5 text-[#FECC0E]" />,
        title: 'Risk analysis engine',
        description: 'Powerful analysis capabilities to evaluate risk factors'
      });
    }
    ...
    if (features.length < 4) {
      features.push({
        ...
        title: 'Risk identification',
        description: 'Systematic approach to identify potential risks'
      });
    }
    if (features.length < 4) {
      features.push({
        ...
        description: 'Evaluate the likelihood and impact of identified risks'
      });
    }
    if (features.length < 4) {
      features.push({
        ...
        description: 'Develop and implement strategies to address risks'
      });
    }
    if (features.length < 4) {
      features.push({
        icon: <Bell className="h-5 w-5 text-[#FECC0E]" />,
        title: 'Risk alerts',
        description: 'Automatic notifications when risk thresholds are exceeded'
      });
    }
    return features;
  };
  ...
  const getFAQs = () => {
    ...
    faqs.push({
      ...
      answer: tool.department ? `This tool is designed primarily for ${tool.department} teams, but can be utilized by other departments with appropriate permissions.` : tool.industry ? `This tool is designed for risk management professionals in the ${tool.industry} industry, particularly those involved in risk assessment and mitigation.` : 'This tool is designed for risk management professionals, compliance officers, and team leaders looking to optimize risk management processes and ensure regulatory compliance.'
    });
    // What data is required?
    faqs.push({
      question: 'What data is required to use this tool?',
      answer: tool.realTime ? 'This tool requires access to real-time risk data, including process metrics, control statuses, and historical risk events.' : 'This tool requires historical risk data, including past risk assessments, incident reports, and control effectiveness information.'
    });
    ...
    faqs.push({
      ...
      answer: tool.easeOfUse === 'Advanced' ? 'This tool requires advanced knowledge of risk management concepts and some data analysis skills.' : tool.easeOfUse === 'Intermediate' ? 'This tool requires basic understanding of risk management concepts but is designed to be accessible to non-technical users with proper training.' : 'This tool is designed to be user-friendly and requires minimal technical expertise to operate effectively.'
    });
    // What are the main benefits?
    faqs.push({
      question: 'What are the main benefits of using this tool?',
      answer: tool.aiPowered ? 'The main benefits include AI-powered risk insights, automated risk identification, predictive analytics, and data-driven decision making for risk management.' : 'The main benefits include increased risk visibility, systematic risk assessment, efficiency improvements in risk mitigation, and data-driven decision making for compliance and governance.'
    });
    return faqs;
  };
  ...
  const getFeatureIcon = (feature: string | null) => {
    if (feature === 'Risk Mitigation' || feature === 'Risk Analysis') {
      return <Shield className="h-24 w-24 text-gray-400" />;
    } else if (feature === 'Heatmap') {
      return <AlertOctagon className="h-24 w-24 text-gray-400" />;
    } else if (feature === 'Strategy Alignment' || feature === 'Alignment') {
      ...
      return <Scale className="h-24 w-24 text-gray-400" />;
    }
  };
      ...
      <div className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 ${comparisonTools.length > 0 ? 'pt-28' : 'pt-8'}`}>
        ...
        <div className="mt-8 flex flex-col lg:flex-row gap-6">
          ...
          <div className="lg:w-3/4">
                          ...
                          <div>
                            ...
                            <ul className="space-y-2 text-sm text-gray-600">
                              <li className="flex items-start">
                                ...
                                <span>Risk data with historical metrics</span>
                              </li>
                              <li className="flex items-start">
                                <div className="h-5 w-5 rounded-full bg-[#FECC0E]/20 flex items-center justify-center mt-0.5 mr-2">
                                  <span className="h-1.5 w-1.5 rounded-full bg-[#FECC0E]"></span>
                                </div>
                                <span>Process and control information</span>
                              </li>
                              <li className="flex items-start">
                                <div className="h-5 w-5 rounded-full bg-[#FECC0E]/20 flex items-center justify-center mt-0.5 mr-2">
                                  <span className="h-1.5 w-1.5 rounded-full bg-[#FECC0E]"></span>
                    ...
                    {expandedSections.includes(3) && <div className="px-4 pb-4">
                        <div className="bg-gray-50 rounded-lg p-6">
                          ...
                          <p className="text-gray-600 text-sm">
                            ...
                            existing risk data and workflows. The integration
            ...
            {activeTab === 'features' && <div className="space-y-4">
                      ...
                      <div className="flex items-start mt-4">
                        ...
                        <div className="ml-4">
                          ...
                          <p className="mt-2 text-gray-600 text-sm">
                            This{' '}
                            {tool.title.toLowerCase().includes('template') ? 'template' : tool.title.toLowerCase().includes('dashboard') ? 'dashboard' : tool.title.toLowerCase().includes('analyzer') ? 'analyzer' : 'risk management tool'}{' '}
                            is designed specifically for{' '}
                            {tool.industry || tool.department || 'organizational'}{' '}
                            environments, providing{' '}
                            {tool.aiPowered ? 'intelligent, AI-powered risk insights' : 'comprehensive risk management capabilities'}
                            {tool.realTime ? ' in real-time' : ''}. With{' '}
                            {tool.feature ? `advanced ${tool.feature.toLowerCase()} functionality` : 'powerful features'}
                            , you can optimize your risk management processes
                            and{' '}
                            {tool.useCase ? `improve ${tool.useCase.toLowerCase()} workflows` : 'enhance risk identification and mitigation'}
                            .
                          </p>
                        </div>
                      </div>
                ...
                {/* Risk Management Framework Accordion */}
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                  ...
                  {expandedSections.includes(11) && <div className="px-4 pb-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                        <div className="bg-white rounded-lg border border-gray-200 p-5">
                          <div className="h-10 w-10 rounded-full bg-[#FECC0E]/10 flex items-center justify-center mb-3">
                            <AlertTriangle className="h-5 w-5 text-[#FECC0E]" />
                          </div>
                          <h3 className="font-medium text-gray-900 mb-2">
                            Risk Identification
                          </h3>
                          <p className="text-gray-600 text-sm">
                            Systematic approach to identify potential risks
                            before they impact business operations
                          </p>
                        </div>
                        <div className="bg-white rounded-lg border border-gray-200 p-5">
                          <div className="h-10 w-10 rounded-full bg-[#FECC0E]/10 flex items-center justify-center mb-3">
                            <Scale className="h-5 w-5 text-[#FECC0E]" />
                          </div>
                          <h3 className="font-medium text-gray-900 mb-2">
                            Risk Assessment
                          </h3>
                          <p className="text-gray-600 text-sm">
                            Evaluate the likelihood and impact of identified
                            risks to prioritize mitigation efforts
                          </p>
                        </div>
                        <div className="bg-white rounded-lg border border-gray-200 p-5">
                          <div className="h-10 w-10 rounded-full bg-[#FECC0E]/10 flex items-center justify-center mb-3">
                            <Shield className="h-5 w-5 text-[#FECC0E]" />
                          </div>
                          <h3 className="font-medium text-gray-900 mb-2">
                            Risk Mitigation
                          </h3>
                          <p className="text-gray-600 text-sm">
                            Develop and implement strategies to address and
                      ...
                      <ul className="space-y-3 mt-4">
                        <li className="flex items-start">
                          ...
                          <span className="text-gray-600 text-sm">
                            Start with a small pilot area before full
                        ...
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Involve risk management stakeholders early in the
                            implementation process
                          </span>
                        </li>
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Ensure risk thresholds and metrics are clearly
                ...
                <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
                  ...
                  {expandedSections.includes(12) && <div className="px-4 pb-4">
                      <p className="text-gray-600 text-sm mt-4">
                        Implementing a new risk management tool requires
                  ...
                  {expandedSections.includes(13) && <div className="px-4 pb-4">
                      <ul className="space-y-3 mt-4">
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Establish clear risk thresholds and tolerance levels
                        ...
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Conduct regular risk assessments rather than waiting
                            for annual reviews
                          </span>
                        </li>
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Focus on both risk prevention and contingency
                            planning
                          </span>
                        </li>
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Use data-driven insights to inform risk management
                            decisions
                          </span>
                        </li>
                        <li className="flex items-start">
                          <CheckCircle className="h-5 w-5 text-[#9E800D] mr-3 flex-shrink-0 mt-0.5" />
                          <span className="text-gray-600 text-sm">
                            Train all stakeholders on risk identification and
        ...
        {relatedTools.length > 0 && <div className="mt-16">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {relatedTools.map(relatedTool => <Link key={relatedTool.id} to={`/risk-management/${relatedTool.id}`} className="group">
                    ...
                    <div className="p-6 flex flex-col h-full">
                      ...
                      <div className="border-t border-gray-100 pt-4 mt-auto flex justify-between items-center">
                        <span className="text-xs text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                          {relatedTool.industry || relatedTool.useCase || relatedTool.feature || 'Risk Management'}
                        </span>
